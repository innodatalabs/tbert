...
class Transformer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.dense_layer_norm = torch.nn.LayerNorm(hidden_size, eps=1.e-9)
        self.intermediate = torch.nn.Linear(hidden_size, intermediate_size)
        self.output = torch.nn.Linear(intermediate_size, hidden_size)
        self.output_layer_norm = torch.nn.LayerNorm(hidden_size, eps=1.e-9)

    def forward(self, inp, inp_mask, batch_size=1):
        '''
        B - batch size
        S - sequence length
        H - hidden size
        ...
        inp_mask - an int tensor of shape [B, S, S] defining attention mask
        ...
        '''
        # --> [B*S, H]
        x = self.attention(inp, inp, inp, inp_mask, batch_size=batch_size)
        # --> [B*S, H]
        x = self.dense(x)
        x = self.dropout(x)
